...
SCRIPT_DIR=$PWD
LOG_FILE="$LOGS_FOLDER/$SCRIPT_NAME.log" #full path - /var/log/shell-script/dispatch.log
...
VALIDATE $? "Install dependencies go mod init"

go get &>>$LOG_FILE
VALIDATE $? "Install dependencies go get"

go build &>>$LOG_FILE
VALIDATE $? "Install dependencies go build"

cp $SCRIPT_DIR/dispatch.service /etc/systemd/system/dispatch.service &>>$LOG_FILE # its better to give full path while copying because we dont know where we are present right now, hence when we provide full path then we don't face any issues.
