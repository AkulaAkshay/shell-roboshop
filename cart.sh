...
SCRIPT_DIR=$PWD
LOG_FILE="$LOGS_FOLDER/$SCRIPT_NAME.log" #full path - /var/log/shell-script/cart.log
...
cp $SCRIPT_DIR/cart.service /etc/systemd/system/cart.service &>>$LOG_FILE # its better to give full path while copying 
